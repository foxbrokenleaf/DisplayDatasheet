...
/**
  ...
  *           范围：OLED_8X16		宽8像素，高16像素
  *                 OLED_6X8		宽6像素，高8像素
  * 参    数：format 指定要显示的格式化字符串，范围：ASCII码可见字符
  * 参    数：... 格式化字符串参数列表
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  * 新增功能：支持自动换行，超出当前行宽度或屏幕边界时自动换到下一行
  */
void OLED_Printf_New(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
    ...
    int16_t currentX = X;                    //当前打印位置X坐标
    int16_t currentY = Y;                    //当前打印位置Y坐标
    uint8_t charWidth = 0;                    //字符宽度
    uint8_t charHeight = 0;                    //字符高度
    uint8_t maxLineChars = 0;                 //每行最大字符数
    
    // 根据字体大小设置字符宽度、高度和每行最大字符数
    if (FontSize == OLED_8X16)
    {
        charWidth = 8;
        charHeight = 16;
        maxLineChars = 16;                    // 128 / 8 = 16个字符
    }
    else if (FontSize == OLED_6X8)
    {
        charWidth = 6;
        charHeight = 8;
        maxLineChars = 21;                    // 128 / 6 ≈ 21.3，取21个字符
    }
    else
    {
        // 默认处理
    ...
    for (int i = 0; String[i] != '\0'; i++)
    {
        ...
        {
            currentX = X;                    // 回到起始X坐标
            currentY += charHeight;          // 换到下一行
            
            // 检查是否超出屏幕下边界
            if (currentY + charHeight > 64)  // 超出64像素高度
            {
                // 可以滚动显示或者停止显示
                // 这里选择停止显示
                break;
        ...
        // 检查是否需要自动换行（超出右边界或达到最大字符数）
        if (currentX + charWidth > 128 ||    // 超出屏幕右边界
            ((currentX - X) / charWidth) >= maxLineChars) // 达到每行最大字符数
        {
            currentX = X;                    // 回到起始X坐标
            currentY += charHeight;          // 换到下一行
        ...
        // 显示字符（只处理ASCII字符，0x20-0x7E为可见字符）
        if (String[i] >= 0x20 && String[i] <= 0x7E)
        {
            OLED_ShowChar(currentX, currentY, String[i], FontSize);
            currentX += charWidth;
        }
        else if (String[i] == '\t')          // 制表符处理（替换为4个空格）
        {
            for (int j = 0; j < 4; j++)
            {
                // 检查是否需要换行
                if (currentX + charWidth > 128)
                {
                    currentX = X;
                    currentY += charHeight;
                    if (currentY + charHeight > 64) break;
                }
                OLED_ShowChar(currentX, currentY, ' ', FontSize);
                currentX += charWidth;
            }
        }
    }
